...
function getAPICfg() {
  ...
  return {
    ...
    cid: (document.getElementById('cfg-cid')?.value?.trim() || saved.cid || ''),
    cs: (document.getElementById('cfg-cs')?.value?.trim() || saved.cs || ''),
    reg: (document.getElementById('cfg-reg')?.value || saved.reg || 'us'),
    loc: (document.getElementById('cfg-loc')?.value || saved.loc || 'en_US'),
...
function hasAPICfg() {
  ...
  return !!(a.proxy && a.cid && a.cs && a.realm && a.guild);
}
...
function proxyPost(proxy, url, body) {
  return pbUrl(proxy) + '?url=' + encodeURIComponent(url) + '&body=' + encodeURIComponent(body);
}

async function getToken(cfg) {
  var r = await fetch(proxyPost(
    cfg.proxy,
    'https://' + cfg.reg + '.battle.net/oauth/token',
    'grant_type=client_credentials&client_id=' + encodeURIComponent(cfg.cid) + '&client_secret=' + encodeURIComponent(cfg.cs)
  ));
...
// ── Skeleton loading for roster table ─────────────────────
...
async function fetchAPI(silent) {
  ...
  var cfg = getAPICfg();
  if (!cfg.proxy || !cfg.cid || !cfg.cs || !cfg.realm || !cfg.guild) {
...
async function refreshExisting() {
  var cfg = getAPICfg();
  if (!cfg.proxy || !cfg.cid || !cfg.cs) return;
  var btn = document.getElementById('btnRefresh');
...
async function trackChar() {
  ...
  if (!cfg.proxy || !cfg.cid || !cfg.cs) { notify('Configure a API.'); return; }

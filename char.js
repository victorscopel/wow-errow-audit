...
function buildSidebar(c, id) {
    ...
    return '<div class="char-header-card cls-bg-' + clsSlug + '">' +
        ...
        '</div>' +
        '<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-bottom:10px">' + roleSel + '<span class="ilvl ' + ilvlC(c.ilvl) + '">' + (c.ilvl || '—') + '</span>' + (c.mythicRating ? '<span style="font-weight:700;color:' + ratingCol(c.mythicRating) + '">M+ ' + c.mythicRating + '</span>' : '') + (c.gear ? tierBadge(c) + embBadge(c) : '') + '</div>' +
